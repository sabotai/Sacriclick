...
public class MasterWaypointer : MonoBehaviour {
	...
	void FixedUpdate () {
		if (!CraneGame.beginCraneGame){
			...
			if(failed) {
				releaseVics = true;
				if (!sacrificer.GetComponent<Sacrifice>().failed) sacrificer.GetComponent<Sacrifice>().Fail(2f, "NEVER SACRIFICE WITHOUT CONSENT!");// "You sacrificed someone without their consent!");
			} else {
